...
* VX      R*4    Single precision array - version ORA21                 *
* KX      I*4    Integer array          - version ORA31                 *
...
************************************************************************
C
      SUBROUTINE ORA11(DX,ARR,MFILE,IFMT)
C
C *** Double precision version
C
      IMPLICIT DOUBLE PRECISION (A,C-H,O-U,W-Z),LOGICAL(B)
      CHARACTER SUB*6,FMT*15,CPARAM*120
      CHARACTER ARR*6,CFORM*15,ARR2*6
      DIMENSION DX(*)
...
C
      SUB='ORA1'
      IF (ICHECK.GE.997) CALL OTRC('ORA11 ','03/21/89')
C
      IER=0
      BFMT=IFMT.EQ.1
c      goto 123
C
     ...
     *       ARR2,CFORM,ITYPE,ILEN
      ELSE
       READ (MFILE,ERR=99997,END=99997) ARR2,ITYPE,ILEN,ILEN8,JRECL
      ENDIF
C
...
C
C
      SUBROUTINE ORA21(VX,ARR,MFILE,IFMT)
C
C *** Double precision version
C
      IMPLICIT DOUBLE PRECISION (A,C-H,O-U,W-Z),LOGICAL(B)
...
C
      SUB='ORA21'
      IF (ICHECK.GE.997) CALL OTRC('ORA21 ','03/21/89')
C
      IER=0
      BFMT=IFMT.EQ.1
C
...
C
C
      SUBROUTINE ORA31(KX,ARR,MFILE,IFMT)
C
C *** Integer version
C
      IMPLICIT DOUBLE PRECISION (A,C-H,O-U,W-Z),LOGICAL(B)
...
C
      SUB='ORA31'
      IF (ICHECK.GE.997) CALL OTRC('ORA31  ','03/21/89')
C
      IER=0
      BFMT=IFMT.EQ.1
C
