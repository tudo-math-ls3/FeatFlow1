...
C *** Coefficient of stiffness matrix, right hand side, exact solution
      EXTERNAL COEFNN,RHS,UE,PE,UEX,UEY
...
C
      CALL ZTIME(TTT0)
      IF (IRHS.EQ.0) THEN
       CALL LCL1(DWORK(KF1),NUPT)
      ENDIF
C
      IF (IRHS.EQ.1) THEN
       CALL LLC1(DWORK(L(LRHS)),DWORK(KF1),NUPT,TSTEPB,0D0)
c
c       goto 1750
       DO 174 INU=1,NU
         DWORK(KF1+INU-1)=DWORK(KF1+INU-1)
     *        +TSTEPB*(1D0-VOLEXK*DWORK(KT+INU-1))*DWORK(KM1+INU-1)*g(1)
         DWORK(KF2+INU-1)=DWORK(KF2+INU-1)
     *        +TSTEPB*(1D0-VOLEXK*DWORK(KT+INU-1))*DWORK(KM1+INU-1)*g(2)
174    CONTINUE
c
1750  ENDIF
C
...
C
       ...
       IF (IUPW.EQ.1) THEN
            if (IPRECA.eq.4) then
               IF (IELT.EQ.0) 
     *              CALL ADDSTP(DWORK(KU1),DWORK(KU2),DWORK(KU1),
     *           DWORK(KU2),1D0,0D0,DWORK(KU1),DWORK(KU2),
     *              DWORK(KF1),DWORK(KF2),VWORK(KA1),NA,KWORK(KCOLA),
     *              KWORK(KLDA),KWORK(L(LVERT)),KWORK(L(LMID)),
     *              DWORK(L(LCORVG)),E031,COEFNN,2,3,-1D0,DWORK(kny))
               IF (IELT.EQ.1) 
     *              CALL ADDSTP(DWORK(KU1),DWORK(KU2),DWORK(KU1),
     *           DWORK(KU2),1D0,0D0,DWORK(KU1),DWORK(KU2),
     *              DWORK(KF1),DWORK(KF2),VWORK(KA1),NA,KWORK(KCOLA),
     *              KWORK(KLDA),KWORK(L(LVERT)),KWORK(L(LMID)),
     *              DWORK(L(LCORVG)),E030,COEFNN,2,3,-1D0,DWORK(kny))
               IF (IELT.EQ.2) 
     *              CALL ADDSTN(DWORK(KU1),DWORK(KU2),DWORK(KU1),
     *           DWORK(KU2),1D0,0D0,DWORK(KU1),DWORK(KU2),
     *              DWORK(KF1),DWORK(KF2),VWORK(KA1),NA,KWORK(KCOLA),
     *              KWORK(KLDA),KWORK(L(LVERT)),KWORK(L(LMID)),
     *              DWORK(L(LCORVG)),EM31,COEFNN,2,3,-1D0,DWORK(kny))
               IF (IELT.EQ.3) 
     *              CALL ADDSTN(DWORK(KU1),DWORK(KU2),DWORK(KU1),
     *           DWORK(KU2),1D0,0D0,DWORK(KU1),DWORK(KU2),
     *              DWORK(KF1),DWORK(KF2),VWORK(KA1),NA,KWORK(KCOLA),
     *              KWORK(KLDA),KWORK(L(LVERT)),KWORK(L(LMID)),
     *              DWORK(L(LCORVG)),EM30,COEFNN,2,3,-1D0,DWORK(kny))
            endif
            if(ISTOK.NE.1)
     *          CALL GUPWD (DWORK(KU1),DWORK(KU2),DWORK(KU1),
     *           DWORK(KU2),1D0,0D0,DWORK(KU1),DWORK(KU2),
     *           DWORK(KF1),DWORK(KF2),VWORK(KA1),NA,KWORK(KCOLA),
     *           KWORK(KLDA),KWORK(L(LVERT)),KWORK(L(LMID)),
     *           DWORK(L(LCORVG)),2,3,DWORK(kny))
       ELSE
        IF (IELT.EQ.0) 
     ...
     *               DWORK(KF1),DWORK(KF2),VWORK(KA1),NA,KWORK(KCOLA),
     *               KWORK(KLDA),KWORK(L(LVERT)),KWORK(L(LMID)),
     *               DWORK(L(LCORVG)),E031,COEFNN,2,3,0D0,DWORK(kny))
        IF (IELT.EQ.1) 
     *   CALL SUPWDG(DWORK(KU1),DWORK(KU2),DWORK(KU1),DWORK(KU2),
     *               1D0,0D0,DWORK(KU1),DWORK(KU2),
     *               DWORK(KF1),DWORK(KF2),VWORK(KA1),NA,KWORK(KCOLA),
     *               KWORK(KLDA),KWORK(L(LVERT)),KWORK(L(LMID)),
     *               DWORK(L(LCORVG)),E030,COEFNN,2,3,0D0,DWORK(kny))
        IF (IELT.EQ.2) 
     *   CALL SUPWNP(DWORK(KU1),DWORK(KU2),DWORK(KU1),DWORK(KU2),
     *               1D0,0D0,DWORK(KU1),DWORK(KU2),
     *               DWORK(KF1),DWORK(KF2),VWORK(KA1),NA,KWORK(KCOLA),
     *               KWORK(KLDA),KWORK(L(LVERT)),KWORK(L(LMID)),
     *               DWORK(L(LCORVG)),EM31,COEFNN,2,3,0D0,DWORK(kny))
        IF (IELT.EQ.3) 
     *   CALL SUPWNP(DWORK(KU1),DWORK(KU2),DWORK(KU1),DWORK(KU2),
     *               1D0,0D0,DWORK(KU1),DWORK(KU2),
     *               DWORK(KF1),DWORK(KF2),VWORK(KA1),NA,KWORK(KCOLA),
     *               KWORK(KLDA),KWORK(L(LVERT)),KWORK(L(LMID)),
     *               DWORK(L(LCORVG)),EM30,COEFNN,2,3,0D0,DWORK(kny))
       ENDIF
       THSTEP=TOSTEP
       CALL ZTIME(TTT1)
       TTUPW=TTUPW+TTT1-TTT0
...
C
      ...
      APDIV=PRDIF2*TMSTEP
      IF (APDIV.NE.0D0) 
     *    CALL XDFKDV(KP,KAUXP,VWORK(L(KLAREA(NLEV))),
     *                NP,APDIV,DWORK(kny))
      CALL ZTIME(TTT1)
      TTADF=TTADF+TTT1-TTT0
C ************************************************************************
...
C ************************************************************************
      THSTEP=TSAVST
      CALL TCALC (MFILE,TMSTEP)
      THSTEP=TSTEPH
C
      DMAXT=0D0
      DMINT=0D0
      DO 9001 INU=1,NU
      DMINT=MIN(DMINT,DWORK(KT+INU-1))
9001  DMAXT=MAX(DMAXT,DWORK(KT+INU-1))
c
      write(6,*) '-> T range:',TIMENS,DMAXT,DMINT
      goto 9005
      write(6,*) '-> T bounded:',TIMENS,DMAXT,DMINT
C
      DO 9003 INU=1,NU
      IF (DWORK(KT+INU-1).GT.1.1D0) DWORK(KT+INU-1)=1.1D0
      IF (DWORK(KT+INU-1).LT.-0.1D0) DWORK(KT+INU-1)=-0.1D0
9003  CONTINUE
C
C
C ************************************************************************
9005  IF (MSHOW.GE.2) WRITE(MTERM,3)
