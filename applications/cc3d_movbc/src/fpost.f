...
c$$$     *  CALL  BDFORC(DWORK(KU1),DWORK(KU2),DWORK(KU3),DWORK(KPL),
c$$$     *               KWORK(L(LVERT)),KWORK(L(LAREA)),KWORK(L(LEDGE)),
c$$$     *               KWORK(L(KLABD(NLEV))),KWORK(L(KELBD(NLEV))),
c$$$     *               DWORK(L(LCORVG)),EM31,DFW,DAW)
c$$$C
c$$$       IF ((IELT.EQ.1).OR.(IELT.EQ.3)) 
c$$$     *  CALL  BDFORC(DWORK(KU1),DWORK(KU2),DWORK(KU3),DWORK(KPL),
c$$$     *               KWORK(L(LVERT)),KWORK(L(LAREA)),KWORK(L(LEDGE)),
c$$$     *               KWORK(L(KLABD(NLEV))),KWORK(L(KELBD(NLEV))),
c$$$     *               DWORK(L(LCORVG)),EM30,DFW,DAW)
c$$$C
c$$$       IF (ITYP.EQ.1) THEN
c$$$         IF (MSHOW.GE.2) 
c$$$     *       WRITE(MTERM,1004) DFW,DAW
c$$$         IF (MSHOW.GE.1) 
c$$$     *       WRITE(MFILE,1004) DFW,DAW
c$$$c         WRITE(53,2000) REAL(TIMENS),REAL(DFW),REAL(DAW)
c$$$      ENDIF
C
C=======================================================================
...
C
     ...
     *  CALL  CBDFNEW(DWORK(KU1),DWORK(KU2),DWORK(KU3),DWORK(KP),
     *               KWORK(L(LVERT)),KWORK(L(LAREA)),KWORK(L(LEDGE)),
     *               KWORK(L(KLABD(NLEV))),KWORK(L(KELBD(NLEV))),
     *               DWORK(L(LCORVG)),EM31,DFWX,DFWY,DFWZ)
C
       IF ((IELT.EQ.1).OR.(IELT.EQ.3)) 
     *  CALL  CBDFNEW(DWORK(KU1),DWORK(KU2),DWORK(KU3),DWORK(KP),
     *               KWORK(L(LVERT)),KWORK(L(LAREA)),KWORK(L(LEDGE)),
     *               KWORK(L(KLABD(NLEV))),KWORK(L(KELBD(NLEV))),
     *               DWORK(L(LCORVG)),EM30,DFWX,DFWY,DFWZ)
C
      IF (ITYP.EQ.1) THEN
       IF (MSHOW.GE.2) 
     *  WRITE(MTERM,'(A,3(E16.5))') 'I(FORCE) BDR CP ',DFWX,DFWY,DFWZ
       IF (MSHOW.GE.1) 
     *  WRITE(MFILE,'(A,3(E16.5))') 'I(FORCE) BDR CP ',DFWX,DFWY,DFWZ
       WRITE(53,2001) REAL(TIMENS),REAL(DFWX),REAL(DFWY),REAL(DFWZ)
      ENDIF
C=======================================================================
C *** write lift and drag 
C=======================================================================
C
       IF ((IELT.EQ.0).OR.(IELT.EQ.2)) 
     *  CALL  LBDFNEW(DWORK(KU1),DWORK(KU2),DWORK(KU3),DWORK(KPL),
     *               KWORK(L(LVERT)),KWORK(L(LAREA)),KWORK(L(LEDGE)),
     *               KWORK(L(KLABD(NLEV))),KWORK(L(KELBD(NLEV))),
     *               DWORK(L(LCORVG)),EM31,DFWX,DFWY,DFWZ)
C
...
C
      IF (ITYP.EQ.1) THEN
       IF (MSHOW.GE.2) 
     *  WRITE(MTERM,'(A,3(E16.5))') 'I(FORCE) BDR LP ',DFWX,DFWY,DFWZ
       IF (MSHOW.GE.1) 
     *  WRITE(MFILE,'(A,3(E16.5))') 'I(FORCE) BDR LP ',DFWX,DFWY,DFWZ
c       WRITE(53,2001) REAL(TIMENS),REAL(DFWX),REAL(DFWY),REAL(DFWZ)
      ENDIF
C=======================================================================
C *** write lift and drag by volume integration
C=======================================================================
C
      CALL ZNEW (nat,1,lal,'al   ')

       IF (IELT.EQ.0) 
     *  CALL  BDFVOL(DWORK(KU1),DWORK(KU2),DWORK(KU3),DWORK(KP),
     *               KWORK(L(LVERT)),KWORK(L(LAREA)),KWORK(L(LEDGE)),
     *               KWORK(L(KLABD(NLEV))),KWORK(L(KELBD(NLEV))),
     *               DWORK(L(LCORVG)),E031,DFWX,DFWY,DFWZ,
     *               DWORK(L(lal)),0)
       IF (IELT.EQ.1) 
     *  CALL  BDFVOL(DWORK(KU1),DWORK(KU2),DWORK(KU3),DWORK(KP),
     *               KWORK(L(LVERT)),KWORK(L(LAREA)),KWORK(L(LEDGE)),
     *               KWORK(L(KLABD(NLEV))),KWORK(L(KELBD(NLEV))),
     *               DWORK(L(LCORVG)),E030,DFWX,DFWY,DFWZ,
     *               DWORK(L(lal)),0)
       IF (IELT.EQ.2) 
     *  CALL  BDFVOL(DWORK(KU1),DWORK(KU2),DWORK(KU3),DWORK(KP),
     *               KWORK(L(LVERT)),KWORK(L(LAREA)),KWORK(L(LEDGE)),
...
C
      ...
      IF (ITYP.EQ.1) THEN
       IF (MSHOW.GE.2) 
     *  WRITE(MTERM,'(A,3(E16.5))') 'I(FORCE) VOL CP ',DFWX,DFWY,DFWZ
       IF (MSHOW.GE.1) 
     *  WRITE(MFILE,'(A,3(E16.5))') 'I(FORCE) VOL CP ',DFWX,DFWY,DFWZ
       WRITE(54,2001) REAL(TIMENS),REAL(DFWX),REAL(DFWY),REAL(DFWZ)
      ENDIF
C=======================================================================
C *** write lift and drag by volume integration
C=======================================================================
C
      CALL ZNEW (nat,1,lal,'al   ')
       ...
       IF (IELT.EQ.0) 
     *  CALL  BDFVOL(DWORK(KU1),DWORK(KU2),DWORK(KU3),DWORK(KPL),
     *               KWORK(L(LVERT)),KWORK(L(LAREA)),KWORK(L(LEDGE)),
     *               KWORK(L(KLABD(NLEV))),KWORK(L(KELBD(NLEV))),
     *               DWORK(L(LCORVG)),E031,DFWX,DFWY,DFWZ,
     *               DWORK(L(lal)),1)
...
C
      ...
      IF (ITYP.EQ.1) THEN
       IF (MSHOW.GE.2) 
     *  WRITE(MTERM,'(A,3(E16.5))') 'I(FORCE) VOL LP ',DFWX,DFWY,DFWZ
       IF (MSHOW.GE.1) 
     *  WRITE(MFILE,'(A,3(E16.5))') 'I(FORCE) VOL LP ',DFWX,DFWY,DFWZ
c       WRITE(54,2001) REAL(TIMENS),REAL(DFWX),REAL(DFWY),REAL(DFWZ)
      ENDIF
C=======================================================================
C
...
C
1000  FORMAT (6E12.4)
1001  FORMAT ('P(VELO) ',6(E12.4))
1002  FORMAT ('P(PRES) ',4(E12.4))
1003  FORMAT ('I(PRES) ',2(E12.4))
1004  FORMAT ('I(FORCE)',2(E12.4))
2000  FORMAT(3E14.6,2(' '))
2001  FORMAT(4(E14.6),1(' '))
c
c
c
99999 END
