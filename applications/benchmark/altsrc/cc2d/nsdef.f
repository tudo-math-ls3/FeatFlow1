...
C
     ...
     *                DWORK(L(LD1)),DWORK(L(LD2)),
     *                VWORK(KA1),KWORK(KCOLA),KWORK(KLDA),
     *                KWORK(L(LVERT)),KWORK(L(LMID)),DWORK(L(LCORVG)),0)
         ELSE
          IF (IELT.EQ.0) 
     *     CALL SUPWDG(DWORK(KU1),DWORK(KU2),DWORK(KU1),DWORK(KU2),
     *                 1D0,0D0,DWORK(KU1),DWORK(KU2),
...
C=======================================================================
C
C       What we actually want to solve here is the system:
C
C         [     ] ( Y1 )   ( D1 )
C         [  T  ] ( Y2 ) = ( D2 )
C         [     ] ( YP )   ( D3 )
C
C       Now remember: We chose the same T here for this preconditioning
C       as for the calculation of the defect! Therefore we can rewrite
C       this equation by writing out the defect, which results in:
C
C         [     ] ( Y1 )   ( KF1 )   [     ] ( U1 )
C         [  T  ] ( Y2 ) = ( KF2 ) - [  T  ] ( U2 )
C         [     ] ( YP )   ( KP  )   [     ] ( P  )
C                 ^^^^^^   ^^^^^^^           ^^^^^^
C                 to calc  orig RHS          old sol.
C
...
C         [     ] ( YP+P  )   ( KP  ) 
C
C       So what we really solve here is not the system as originally
C       claimed, but a modified system which gives us as a result the
C       vector ( Y1+U1, Y2+U2, YP+P )^T. The update vector (Y1,Y2,YP)
C       can be extracted from that by subtracting the original
C       (U1,U2,P).
C
C     Warning: This is an old FeatFlow bug. It works fine until one
C      wants to solve the system very exact, e.g. < 10^-5 in the
C      norm of the residuum.
C
      IRELMG=1
      ITMG=ILMIN
