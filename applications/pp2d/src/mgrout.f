...
C
C
C The following line is CORRECT! - the call should never happen!
C This is to prevent a nasty compiler bug with the Intel Fortran
C compiler on Linux. The YLAX17-routine is passed as parameter to IE010
C and called there. It uses the /XYPAR/ Common block to access 
C information about the matrix. We set the XYPAR Common block here
C and use it in YLAX17 - and between both routines the IE010-routine
C is layered. The result is, that the compiler does not connect the 
C /XYPAR/ Common block of YLAX17 to our routine here. So we insert a 
C Dummy-call to YLAX17 here to force the compiler to connect both 
C routines, as the routines then seem to be directly connected.
C This only happens when interprocedural optimization is switched on...
c
c      IF (NEQ.EQ.0) CALL YLAX17(DX,DX,0,0D0,0D0)
c
C-----------------------------------------------------------------------
