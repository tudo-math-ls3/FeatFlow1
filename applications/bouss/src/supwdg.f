...
C
      ...
      DIMENSION KDFG(NNBAS),KDFL(NNBAS),KAB(2,NNAB)
...
C *** user COMMON blocks
     ...
     *               ISMU,ISLU,NSMU,NSLU,ICYCP,ILMINP,ILMAXP,
     *               IINTP,ISMP,ISLP,NSMP,NSLP
C
...
C *** Summing up over all pairs of multiindices
      ...
      IA=KAB(1,IAB)
      IB=KAB(2,IAB)
      AUX=COEFFN(IA,IB,U1L1,U1L2,U2L1,U2L2,A1L,A2L,DELTA,DCMASS)*OM
C
      DO 310 JDOFE=1,IDFL
      DB=DBAS(KDFL(JDOFE),IA)
      DO 310 IDOFE=1,IDFL
      DENTRY(JDOFE,IDOFE)= DENTRY(JDOFE,IDOFE)
     *                    +DB*DBAS(KDFL(IDOFE),IB)*AUX
310   CONTINUE
301   CONTINUE
200   CONTINUE
C
...
C
      IF (IDEF.GT.0) THEN 
                       D1(KDFG(JDOFE))= D1(KDFG(JDOFE))
     *                                 -DENTH*U1(KDFG(IDOFE))
       IF (ICOMP.EQ.2) D2(KDFG(JDOFE))= D2(KDFG(JDOFE))
     *                                 -DENTH*U2(KDFG(IDOFE))
      ENDIF 
C
400   CONTINUE
...
C
C *** Determine local DELTA for streamline-diffusion
      CALL DELTSD(U1L1,U1L2,U2L1,U2L2,A1L,A2L,IEL,DUMAX,
...
C
************************************************************************
      SUBROUTINE  DELTSD  (U1L1,U1L2,U2L1,U2L2,A1L,A2L,IEL,DUMAX,
     *                    DELTA,KVERT,KMID,DCORVG)
************************************************************************
...
C
           CALL HWAHL (HLOCAL,UNORM, DU1, DU2, IEL, KVERT,KIMD,DCORVG)
