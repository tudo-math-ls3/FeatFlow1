...
C
      ...
      ENDIF
C
      CALL ZNEW(NMT,3,KLMBD(II),'KMBD  ')
      CALL ZNEW(NMT,1,KLDBD(II),'DDBD  ')
      IF (IER.NE.0) GOTO 99999
     ...
     *            DWORK(L(KLCVG(II))),KWORK(L(KLADJ(II))))
      IF (IER.NE.0) GOTO 99999
      KNMBD(II)=NMBD      
11    CONTINUE
C
...
C
     ...
     *             DWORK(L(KLDBD(NLEV))),KWORK(L(KLNPR(NLEV))),
     *             KNMBD(NLEV),NVT,PARX,PARY,UE,
     *             DWORK(L(LCORVG)),KWORK(L(KLADJ(NLEV))),NEL,
     *             KWORK(L(KLMID(NLEV))),KWORK(L(KLVERT(NLEV))))
C
