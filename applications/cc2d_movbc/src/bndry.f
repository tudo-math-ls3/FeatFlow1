...
*-----------------------------------------------------------------------
      ...
      DIMENSION KNPR(*),DDBD(*),DMBDP(*),DCORVG(2,*),KADJ(4,*)
...
C
      IF (KADJ(IVE,IEL).LT.IEL) THEN
       ...
       XX=DCORVG(1,IVT1)
       YY=DCORVG(2,IVT1)
       DIST1=SQRT((XX-XM1)**2+(YY-YM1)**2)
C
       IF (DIST1.LE.0.05D0) THEN
        KNPR(IVT1)=11
       ENDIF
C
       IVE1=IVE+1
...
C
       ...
       XX2=0.5D0*(XX+XX1)
       YY2=0.5D0*(YY+YY1) 
       DIST1=SQRT((XX2-XM1)**2+(YY2-YM1)**2)
C
       IF (DIST1.LE.0.05D0) THEN
        IM1=KMID(IVE,IEL)
        KNPR(IM1)=-2
...
C
     ...
     *          KVERT(NNVE,*),KMID(NNVE,*)
      DIMENSION KMBD(*),KNPR(*),DCORVG(2,*),KADJ(4,*)
