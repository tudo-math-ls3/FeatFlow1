...
************************************************************************
*
*   Purpose: - solver for the nonstationary incompressible Navier Stokes
*              equations by time adaptive projection methods
...
C
      CALL ZTIME(TTT0)
      CALL ZINIT(NNWORK,'feat.msg','#data/PP3D.ERR','#data/PP3D.PRT',
     *                             '#data/PP3D.SYS','#data/PP3D.TRC') 
      CALL ZTIME(TTT1)
      TTLC=TTLC+TTT1-TTT0
C
      MDATA=79
...
C
      IF (ITEXL.NE.0) THEN
       CALL ZNEW(3*NU,-1,LTML ,'DMTL  ')
       IF (IER.NE.0) GOTO 99998
       IF (ABS(IADTIM).EQ.3) THEN
        CALL ZNEW(3*NU,-1,LTMLO,'DMTLO ')
        IF (IER.NE.0) GOTO 99998
        CALL LCP1(DWORK(KU1),DWORK(L(LTML)),3*NU)
       ENDIF
      ENDIF
C
      IF (IADTIM.NE.0) THEN
...
C
      ...
      IF ((ITEXL.NE.0).AND.(ABS(IADTIM).EQ.3)) THEN
       CALL LCP1(DWORK(L(LTML)),DWORK(L(LTMLO)),3*NU)
       TIMO31=TIML31
       TIMO32=TIML32
       TIMO11=TIML11
       TIMO12=TIML12
...
C=======================================================================
      IF ((BSTOP).AND.(ITNSR.LE.NNITR)) THEN
       ...
       IF (ITEXL.NE.0) THEN
        CALL LCP1(DWORK(L(LTMLO)),DWORK(L(LTML)),3*NU)
        TIML31=TIMO31
        TIML32=TIMO32
        TIML11=TIMO11
        TIML12=TIMO12
...
C
      IF ((IEPSAD.GE.1).OR.(IEPSAD.EQ.-1)) THEN
       ...
       RELU20=DSXN/(SQRT(DBLE(3*NU)))
       IF (RELU20.LE.1D0) RELU20=1D0
       CALL LL21(DWORK(L(LD1)),3*NU,DSXN)
       RELU21=DSXN/(SQRT(DBLE(3*NU))*RELU20)
      ENDIF
C
      IF ((IEPSAD.GE.1).OR.(IEPSAD.EQ.-2)) THEN
       CALL LLI1(DWORK(KU1   ),3*NU,DSXN,INDMAX)
       RELUM0=DSXN
       IF (RELUM0.LE.1D0) RELUM0=1D0
       CALL LLI1(DWORK(L(LD1)),3*NU,DSXN,INDMAX)
       RELUM1=DSXN/RELUM0
      ENDIF
C
      IF ((IEPSAD.GE.1).OR.(IEPSAD.EQ.-3)) THEN
       ...
       CALL LL21(DWORK(L(LD1)+3*NU),NP,DSXN)
       RELP21=DSXN/(SQRT(DBLE(NP))*RELP20)
      ENDIF
C
      IF ((IEPSAD.GE.1).OR.(IEPSAD.EQ.-4)) THEN
       CALL LLI1(DWORK(KU1   +3*NU),NP,DSXN,INDMAX)
       RELPM0=DSXN
       IF (RELPM0.LE.1D0) RELPM0=1D0
       CALL LLI1(DWORK(L(LD1)+3*NU),NP,DSXN,INDMAX)
       RELPM1=DSXN/RELPM0
      ENDIF
C
C
...
C
      ...
      IF (IERANA.GE.1)  THEN
       ICUBER=IERANA
       IF (IELT.EQ.0) 
     *  CALL ELPQU(DWORK(KU1),DWORK(KU2),DWORK(KU3),KWORK(L(LVERT)),
     *             KWORK(L(LEDGE)),KWORK(L(LAREA)),DWORK(L(LCORVG)),
     *             E031,ICUBER,ILINT,UE,UEX,UEY,UEZ,MFILE)
       IF (IELT.EQ.1) 
     *  CALL ELPQU(DWORK(KU1),DWORK(KU2),DWORK(KU3),KWORK(L(LVERT)),
     *             KWORK(L(LEDGE)),KWORK(L(LAREA)),DWORK(L(LCORVG)),
     *             E030,ICUBER,ILINT,UE,UEX,UEY,UEZ,MFILE)
       IF (IELT.EQ.2) 
     *  CALL ELPQN(DWORK(KU1),DWORK(KU2),DWORK(KU3),KWORK(L(LVERT)),
     *             KWORK(L(LEDGE)),KWORK(L(LAREA)),DWORK(L(LCORVG)),
     *             EM31,ICUBER,ILINT,UE,UEX,UEY,UEZ,MFILE)
       IF (IELT.EQ.3) 
     *  CALL ELPQN(DWORK(KU1),DWORK(KU2),DWORK(KU3),KWORK(L(LVERT)),
     *             KWORK(L(LEDGE)),KWORK(L(LAREA)),DWORK(L(LCORVG)),
     *             EM30,ICUBER,ILINT,UE,UEX,UEY,UEZ,MFILE)
C
       CALL ELPQP(DWORK(KP),DWORK(L(LDP)),KWORK(L(LVERT)),
     *            KWORK(L(LEDGE)),KWORK(L(LAREA)),DWORK(L(LCORVG)),
     *            E010,ICUBER,PE,MFILE,ILINT)
C
       IF (IERANA.NE.1) THEN
        ICUBER=1
       IF (IELT.EQ.0) 
...
C
      CALL LL21(DWORK(L(LD1)),3*NU,DSXN)
      RELU2=DSXN/(SQRT(DBLE(3*NU))*3D0*TSTEPO)
      CALL LL21(DWORK(L(LD1)+3*NU),NP,DSXN)
