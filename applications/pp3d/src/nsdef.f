...
************************************************************************
*   Purpose: - solver for the stationary Burgers
...
C
      CALL ZTIME(TTT0)
      CALL XMADF2(KM1,KMASS1,KST1,KA1,KCOLA,KLDA,L(LD1),L(LD2),L(LD3),
     *            KU1,KU2,KU3,NA,NU,KWORK(L(KLABD(NLEV))),KNABD(NLEV),
     *            THSTEP)
      CALL ZTIME(TTT1)
...
C
     ...
     *                COEFFN,IDEFUP,1D0)
        ENDIF
C
        IF (ITEXL.EQ.3) CALL LCP1(DWORK(KU1),DWORK(L(LTML)),3*NU)
        IF (ITEXL.GT.0) CALL LLC1(DWORK(L(LTML)),DWORK(KU1),3*NU,
     *                            A1L,A2L)
       ELSE
        IF (IUPW.EQ.1) THEN
...
C
      CALL ZTIME(TTT0)
      IF (INLMAX.GT.1) THEN
       CALL RESDFK(DWORK(L(LD1)),DWORK(L(LD2)),DWORK(L(LD3)),
     *             DWORK(KF1),DWORK(KF2),DWORK(KF3),
     *             NU,RESU1,RESU2,RESU3)
       RESOLD=MAX(RESU1,RESU2,RESU3)
       RES0=RESOLD
       RES =RESOLD
       EPSRES=DMPUD*RESOLD
      ELSE
...
C
     ...
     *                VWORK(KA1),NA,KWORK(KCOLA),KWORK(KLDA),
     *                KWORK(L(LVERT)),KWORK(L(LAREA)),KWORK(L(LEDGE)),
     *                KWORK(L(KLINT(ILEV))),DWORK(L(LCORVG)),E031,
     *                COEFFN,IDEFUP,1D0)
         IF (IELT.EQ.1) 
     *    CALL SUPWDG(DWORK(KU1),DWORK(KU2),DWORK(KU3),
     *                DWORK(KU1),DWORK(KU2),DWORK(KU3),1D0,0D0,
     ...
     *                VWORK(KA1),NA,KWORK(KCOLA),KWORK(KLDA),
     *                KWORK(L(LVERT)),KWORK(L(LAREA)),KWORK(L(LEDGE)),
     *                KWORK(L(KLINT(ILEV))),DWORK(L(LCORVG)),E030,
     *                COEFFN,IDEFUP,1D0)
         IF (IELT.EQ.2) 
     *    CALL SUPWNP(DWORK(KU1),DWORK(KU2),DWORK(KU3),
     *                DWORK(KU1),DWORK(KU2),DWORK(KU3),1D0,0D0,
     ...
     *                VWORK(KA1),NA,KWORK(KCOLA),KWORK(KLDA),
     *                KWORK(L(LVERT)),KWORK(L(LAREA)),KWORK(L(LEDGE)),
     *                KWORK(L(KLINT(ILEV))),DWORK(L(LCORVG)),EM31,
     *                COEFFN,IDEFUP,1D0)
         IF (IELT.EQ.3) 
     *    CALL SUPWNP(DWORK(KU1),DWORK(KU2),DWORK(KU3),
     *                DWORK(KU1),DWORK(KU2),DWORK(KU3),1D0,0D0,
     ...
     *                VWORK(KA1),NA,KWORK(KCOLA),KWORK(KLDA),
     *                KWORK(L(LVERT)),KWORK(L(LAREA)),KWORK(L(LEDGE)),
     *                KWORK(L(KLINT(ILEV))),DWORK(L(LCORVG)),EM30,
     *                COEFFN,IDEFUP,1D0)
        ENDIF
       ENDIF
       CALL ZTIME(TTT1)
...
C=======================================================================
C
      CALL  XOPTCN (KU1,KU2,KU3,L(LU1OLD),L(LU2OLD),L(LU3OLD),
     *              KF1,KF2,KF3,L(LD1),L(LD2),L(LD3),
     *              KAUX1,KAUX2,KAUX3,KA1,KCOLA,KLDA,KST1,KM1,KMASS1,
     *              NA,NU,KWORK(L(KLABD(ILEV))),KNABD(ILEV),
     *              KWORK(L(LVERT)),KWORK(L(LAREA)),DWORK(L(LCORVG)),
     *              KWORK(L(LEDGE)),DELU1,DELU2,DELU3,OMEGA,INEUM)
C
C=======================================================================
C *** Unexpected STOP !!!
C=======================================================================
