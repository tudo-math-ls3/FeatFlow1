************************************************************************
      SUBROUTINE XMADF1(KM1,KMASS1,KST1,KA1,KCOLA,KLDA,KB1,KB2,KB3,
     *                  KCOLB,KLDB,KF1,KF2,KF3,KU1,KU2,KU3,KP,NA,NU,
     *                  THSTEP,TSTEPH,IPROJ)
************************************************************************
...
C
       IF (IMASS.EQ.1) THEN
        IF (THSTEP.NE.0D0) THEN
         CALL VADM1(VWORK(KA1),VWORK(KST1),VWORK(KMASS1),NA,THSTEP,1D0)
        ELSE
         CALL LCP2(VWORK(KMASS1),VWORK(KA1),NA)
        ENDIF
       ELSE
        IF (THSTEP.NE.0D0) THEN
         CALL VADM2(VWORK(KA1),VWORK(KST1),NA,THSTEP)
         DO 100 INU=1,NU
...
C
     ...
     *             DWORK(KF1),DWORK(KF2),DWORK(KF3),1D0,1D0)
C
      ENDIF
C
C=======================================================================
C
      IF (IPRECA.EQ.1) THEN
       IF (IMASS.EQ.0) THEN
        IF (THSTEP.NE.0D0) THEN
C
         CALL D1LAX7(DWORK(KST1),KWORK(KCOLA),KWORK(KLDA),NU,
     *               DWORK(KU1),DWORK(KU2),DWORK(KU3),
     *               DWORK(KF1),DWORK(KF2),DWORK(KF3),THSTEP,1D0)
C
         DO 200 INU=1,NU
         DM1=DWORK(KM1+INU-1)
         DWORK(KF1+INU-1)= DWORK(KF1+INU-1)+DM1*DWORK(KU1+INU-1)
         DWORK(KF2+INU-1)= DWORK(KF2+INU-1)+DM1*DWORK(KU2+INU-1)
         DWORK(KF3+INU-1)= DWORK(KF3+INU-1)+DM1*DWORK(KU3+INU-1)
200      CONTINUE
C
         DO 210 INA=1,NA
...
240      VWORK(KA1+INUA)=REAL(DWORK(KM1+INU-1))
     ...
     *               DWORK(KU1),DWORK(KU2),DWORK(KU3),
     *               DWORK(KF1),DWORK(KF2),DWORK(KF3),THSTEP,1D0)
         CALL D1LAX7(DWORK(KMASS1),KWORK(KCOLA),KWORK(KLDA),NU,
     *               DWORK(KU1),DWORK(KU2),DWORK(KU3),
     *               DWORK(KF1),DWORK(KF2),DWORK(KF3),1D0,1D0)
C
         DO 250 INA=1,NA
...
C
       IF (THSTEP.NE.0D0) THEN
        ...
        IF (IER.NE.0) RETURN
C
        CALL VADM2(VWORK(KA1),VWORK(KA1),NA,THSTEP)
       ELSE
        CALL LCL2(VWORK(KA1),NA)
       ENDIF
C
...
C
       IF (THSTEP.NE.0D0) THEN
        ...
        CLOSE (59)
        IF (IER.NE.0) RETURN
        CALL DADM2(DWORK(KA1),DWORK(KA1),NA,THSTEP)
       ELSE
...
410      DWORK(KA1+INUA)=DWORK(KM1+INU-1)
       ...
       ENDIF
C
       CALL D1LAX7(DWORK(KA1),KWORK(KCOLA),KWORK(KLDA),NU,
     *             DWORK(KU1),DWORK(KU2),DWORK(KU3),
     *             DWORK(KF1),DWORK(KF2),DWORK(KF3),1D0,1D0)
C
...
C
     ...
     *               DWORK(KF3),NEL,NVT,NAT,-TSTEPH,1D0)
        ENDIF
       ENDIF
C
C
...
C
************************************************************************
      SUBROUTINE XMADF2(KM1,KMASS1,KST1,KA1,KCOLA,KLDA,KD1,KD2,KD3,
     *                  KU1,KU2,KU3,NA,NU,KABD,NABD,THSTEP)
************************************************************************
      IMPLICIT DOUBLE PRECISION(A,C-H,O-U,W-Z),LOGICAL(B)      
      PARAMETER (NNARR=299,NNLEV=9,NNWORK=1)
      CHARACTER SUB*6,FMT*15,CPARAM*120
       ...
       IF (IMASS.EQ.0) THEN
        IF (THSTEP.NE.0D0) THEN
         IF (INLMAX.GT.1) THEN
          CALL D1LAX7(DWORK(KST1),KWORK(KCOLA),KWORK(KLDA),NU,
     *                DWORK(KU1),DWORK(KU2),DWORK(KU3),
     *                DWORK(KD1),DWORK(KD2),DWORK(KD3),-THSTEP,1D0)
C
          DO 200 INU=1,NU
          DM1=DWORK(KM1+INU-1)
          DWORK(KD1+INU-1)= DWORK(KD1+INU-1)-DM1*DWORK(KU1+INU-1)
          DWORK(KD2+INU-1)= DWORK(KD2+INU-1)-DM1*DWORK(KU2+INU-1)
          DWORK(KD3+INU-1)= DWORK(KD3+INU-1)-DM1*DWORK(KU3+INU-1)
200       CONTINUE
         ENDIF
C
         DO 210 INA=1,NA
...
C
       IF (IMASS.EQ.0) THEN
         ...
         DO 510 INU=1,NU
         INUA=KWORK(KLDA+INU-1)-1
510      VWORK(KA1+INUA)=REAL(DWORK(KM1+INU-1))
       ELSE
         CALL LCL2(VWORK(KA1),NA)
       ENDIF
C
...
C
************************************************************************
      SUBROUTINE XDFKG(KM1,KB1,KB2,KB3,KCOLB,KLDB,KD1,KD2,KD3,
     *                 KU1,KU2,KU3,KP,NU,NP,TSTEPH)
************************************************************************
      IMPLICIT DOUBLE PRECISION(A,C-H,O-U,W-Z),LOGICAL(B)      
      PARAMETER (NNARR=299,NNWORK=1)
      CHARACTER SUB*6,FMT*15,CPARAM*120
...
C
     ...
     *             DWORK(KD3),NEL,NVT,NAT,TSTEPH,0D0)
      ENDIF
C
      CALL  BDRY0 (DWORK(KD1),DWORK(KD2),DWORK(KD3),KWORK(L(LABD)),NABD)
C
      DO 10 IU=1,NU
      DM1=DWORK(KM1+IU-1)
      DWORK(KU1+IU-1)=DWORK(KU1+IU-1)-DWORK(KD1+IU-1)/DM1
      DWORK(KU2+IU-1)=DWORK(KU2+IU-1)-DWORK(KD2+IU-1)/DM1
      DWORK(KU3+IU-1)=DWORK(KU3+IU-1)-DWORK(KD3+IU-1)/DM1
10    CONTINUE
C
C
C
