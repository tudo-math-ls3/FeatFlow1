...
C
     ...
     *             KWORK(L(LAREA)),KWORK(L(LADJ)),
     *             DWORK(KM1),DWORK(KB1),DWORK(KB2),DWORK(KB3),
     *             KWORK(KCOLB),KWORK(KLDB),NEL,NVT,NAT)
C
C=======================================================================
...
C
      CALL ZTIME(TTT0)
      CALL XMADF1(KM1,KMASS1,KST1,KA1,KCOLA,KLDA,KB1,KB2,KB3,KCOLB,KLDB,
     *            KF1,KF2,KF3,KU1,KU2,KU3,KP,NA,NU,THSTEP,TSTEPH,IPROJ)
      CALL ZTIME(TTT1)
      TTADF=TTADF+TTT1-TTT0
C
C
...
C
      CALL ZTIME(TTT0)
      CALL XDFKD(KB1,KB2,KB3,KCOLB,KLDB,KFP,KU1,KU2,KU3,NU,NP,TSTEPH)
      CALL ZTIME(TTT1)
      TTADF=TTADF+TTT1-TTT0
C
      CALL ZTIME(TTT0)
...
C
      CALL ZTIME(TTT0)
      CALL XDFKG(KM1,KB1,KB2,KB3,KCOLB,KLDB,L(LD1),L(LD2),L(LD3),
...
C
      ...
      APDIV=PRDIF2*TMSTEP
      IF (APDIV.NE.0D0) 
     *    CALL XDFKDV(KP,KAUXP,VWORK(L(KLVOL(NLEV))),NP,APDIV)
      CALL ZTIME(TTT1)
      TTADF=TTADF+TTT1-TTT0
C ************************************************************************
C
