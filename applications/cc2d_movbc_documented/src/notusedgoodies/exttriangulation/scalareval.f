...
C Dummy element call to precalculate values if necessary
...
C Clear the return vectors
      ...
      DO I=1,NSOL
        IF (IAND(FCLC(I),1).NE.0) DVAL(I) = 0D0
        IF (IAND(FCLC(I),2).NE.0) DGRX(I) = 0D0
        IF (IAND(FCLC(I),4).NE.0) DGRY(I) = 0D0
      END DO
