...
C               Proceed to the next element in the list
...
C               There's a crucial trick in this loop concerning the first
C               element! Because of the initialisation, KCOL1(IROW)=IROW!
C               This first element in KCOL1 always corresponds to the
C               diagonal element in the row IROW of the matrix, which
C               is only touched in the case IDOFE=JDOFE - exactly the
C               case which is never seen in this loop as this case is
C               skipped above!
C               Therefore we can start checking for the column JCOL from
C               the second element on in the list, which saves one IF-command
C               after the loop!
...
C             Check also the last entry in the row.
C             This is an important check which makes element pairs like Q1/Q1
C             working. The original FEAT routine AP9 doesn't check the first
C             element in the list, therefore Q1/Q1 would result in a
C             corrupt matrix, as the diagonal entries is inserted 2x!
C
C             If we find the column JCOL, cancel the element insertion
