...
C
C           !!!BUG!!! We have an inner edge or NEUMANN BOUNDARY edge!
C           The Neumann boundary is not treated properly here!
C           This bug is compensated by a special boundary handling
C           in the matrix-vector multiplication MATML and in the
C           functions in the file DFKT.F!!!
C
...
C           (for inner nodes, which where each velocity component 
C           couples with two elements:
C
C                IEL                              IEL
C             |--------|             |--------|--------|
C             |        |             |        |        |
...
C           then. Instead of subtracting B*pressure of a neighbour cell,
C           we therefore subtract B*pressure of the current element.
C           --> WARNING!!! THIS IS A BUG!!!
C               The result of this treatment is that B*(pressure of
C               the current cell) is subtracted from the RHS.
C               On such a cell we solve therefore:
C                       A*v + B*p   = f - B*p
C                 <=>   A*v + 2*B*p = f
C               i.e. we accidently double the influence of the B-matrix!
C               This effect is compensated in other routines, though:
C               the matrix vector multiplication routine MATML and
C               the functions in DFKT.F!
C
...
C           vector.
C           Note that JP1 / JP2 are not only the column numbers in the
C           B-matrices but also numbers of elements -- as we have
C           exactly one DOF per element and the numbering is made that
C           way!
...
C       Call the subroutine ELUPDT for element update.
C
...
C           the row in the B-matrix are the same. 
C           --> WARNING!!! THIS IS A BUG!!!
C               The result of this treatment is that B*(pressure of
C               the current cell) is subtracted from the RHS.
C               On such a cell we solve therefore:
