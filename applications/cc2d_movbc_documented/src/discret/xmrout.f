...
C parameters
      ...
      INTEGER NBLOC
      INTEGER KLA(NBLOC,*),KLCOL(*),KLLD(*)
      INTEGER KNA(*),KNEQ(*),KAB(2,NNAB,*),KABN(*)
      INTEGER ICLEAR,ICUB,ISYMM
      LOGICAL BCON(*)

      CHARACTER CARR*12
      DIMENSION CARR(NBLOC,*)
      LOGICAL BSNGL(NBLOC,*)
...
*                                                                      *                                                                      *
* 12.04.2005 Michael Köster                                            *
*   Routine generates and saves the mesh information for all meshes    *
*   1..NLMAX instead of NLMIN..NLMAX. This is necessary because the    *
*   grid adaption routines perhaps work on even lower levels than the  *
*   solver routines. It costs a little bit more memory to save also    *
*   the grids for lower levels, but it will not destroy anything...    *
* 29.06.2005 Michael Köster                                            *
*  With NLVPRF it's able to specify a number of "pre-refinements" that *
*  are performed to obtain level 1.                                    *
************************************************************************
C
      SUBROUTINE XMSB2(IMID,IADJ,IVEL,IDISP,IBDP,
     *                 S2DI,S2DB,PARX,PARY,TMAX,NLVPRF)
...
C local variables
      ...
      INTEGER IADJ0,IBDP0,IADJ,IBDP,IMID,IVEL,IDISP,ILEVH
...
C         pre-refinement of the grid to obtain level 1

          CALL XSB0X(NLVPRF,IMID,IADJ0,IVEL,IDISP,IBDP0,
     *               S2DI,S2DB,PARX,PARY,TMAX)
     
        ELSE

C lowest level; only create+initialize all structures without refinement
...
C next level: use the current structures and perform one refinement,
C re-initialize the structures

       CALL XSB0X(1,IMID,IADJ0,IVEL,IDISP,IBDP0,
     *            S2DI,S2DB,PARX,PARY,TMAX)

C now the new structures are copied/backuped in the following...
...
C
      IF (ILEV.GE.2) 
     * CALL CHCOOR(DWORK(L(LCORVG)),KWORK(L(LVERT)),
     *             KWORK(L(LADJ)),NEL)
************************************************************************
C
      IF (ILEV.GE.1) THEN
C
C *** Save dimensions for all levels between 1 and NLMAX
C
      KNEL(ILEV) =NEL
      KNVT(ILEV) =NVT
      KNMT(ILEV) =NMT
...
* XMORS2
*
* Read precalculated subdivisions of mesh triangulations for
* all levels from hard disc.
*
* In:
*  IRMESH - 1=files are in formatted form, 0=in unformatted form.
*
* The filenames of the subdivisions are hard coded for now in
* the variables CTRIF, see below.
************************************************************************
...
C parameter
      ...
      INTEGER NEQ,NBLOC,ICLEAR,ICUB
      INTEGER LB(*),KB(NNAB,*),KBN(*)
      LOGICAL BCON(*),BSNGL(*)
...
C parameters
      ...
      INTEGER NBLOC,ICUB
      INTEGER KVERT(NNVE,*),KMID(NNVE,*),KOFF(*)
      INTEGER KB(NNAB,*),KBN(*)
      DOUBLE PRECISION DB(*),DCORVG(2,*),COECON(NNDER,*)
      LOGICAL BCON(*)
...
C local variables
      ...
      INTEGER IDER, IBLOC, IBN, IB, IELTYP, IVE, JP,JDOFE,IGLOB
      LOGICAL BFIRST,BCON0
      DOUBLE PRECISION AUX,DJ1,DJ2,DJ3,DJ4,XI1,XI2,XX,YY,OM
