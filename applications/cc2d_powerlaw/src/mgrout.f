...
c$$$       CALL DEXA(DX(1+KOFFX(NLMAX)),DB(1+KOFFB(NLMAX)),
c$$$     *           DD(1+KOFFD(NLMAX)),KNEQ(NLMAX),RHOLMG,EPS2,ITECG)
       CALL YEXA(DX(1+KOFFX(NLMAX)),DB(1+KOFFB(NLMAX)),
...
c      IF (KPRSM(NLMAX).GT.0) 
c     * CALL DPRSM(DX(1+KOFFX(NLMAX)),DB(1+KOFFB(NLMAX)),
c     *            DD(1+KOFFD(NLMAX)),KNEQ(NLMAX),KPRSM(NLMAX))
c
ccC      IF (BTIME) THEN
CCC       CALL ZTIME(TTS1)
...
C
     ...
     *          -1D0,1D0)
C
       CALL RESDU(DD(1+KOFFD(NLMAX))             ,KNU(NLMAX),NLMAX,DEF1)
       CALL RESDU(DD(1+KOFFD(NLMAX)+1*KNU(NLMAX)),KNU(NLMAX),NLMAX,DEF2)
       CALL RESDP(DD(1+KOFFD(NLMAX)+2*KNU(NLMAX)),KNP(NLMAX),NLMAX,DEFP)
       IF ((ABS(ICHDEF).EQ.1).OR.(ABS(ICHDEF).EQ.3)) THEN
        DEF=SQRT(DEF1**2+DEF2**2+DEFP**2)
       ELSE
        DEF=MAX(DEF1,DEF2,DEFP)
       ENDIF
c
...
C
      ...
      ENDIF
C
      CALL  shift (DX(1+KOFFX(NLEV)),NEQ)
C
...
C
     ...
     *           -1D0,1D0)
C
ccc       IF (ILEV.EQ.NLMAX) THEN
ccc       CALL RESDU(DD(1+KOFFD(NLMAX)),KNU(NLMAX),NLMAX,DEF1)
ccc       CALLRESDU(DD(1+KOFFD(NLMAX)+1*KNU(NLMAX)),KNU(NLMAX),NLMAX,DEF2)
ccc       CALLRESDP(DD(1+KOFFD(NLMAX)+2*KNU(NLMAX)),KNP(NLMAX),NLMAX,DEFP)
ccc       IF ((ABS(ICHDEF).EQ.1).OR.(ABS(ICHDEF).EQ.3)) THEN
ccc        DEF=SQRT(DEF1**2+DEF2**2+DEFP**2)
ccc       ELSE
ccc        DEF=MAX(DEF1,DEF2,DEFP)
ccc       ENDIF
c
        IF (BTIME) THEN
         CALL ZTIME(TTD1)
...
C ***  Unexpected STOP !!!
C=======================================================================
ccc       IF ((DEF/DEFOLD.GT.1D9).OR.(DEF/FD.GT.5D9)) THEN
ccc
ccc        BMGEND=.TRUE.
ccc        GOTO 1000
ccc       ENDIF
...
ccc      DEFOLD=DEF
ccc       IF (BREL) THEN
ccc        IF (DEF.LE.FD*EPS1.AND.DEF.LE.EPS2.AND.ITE.GE.NIT0) GOTO 1000
ccc       ELSE
ccc        IF (DEF.LE.EPS2) GOTO 1000
ccc       ENDIF
ccc      ENDIF
C
       ILEV=ILEV-1
...
C ***  Unexpected STOP !!!
C=======================================================================
       IF ((DEF/DEFOLD.GT.5D2).OR.(DEF/FD.GT.5D3)) THEN
        BMGEND=.TRUE.
        GOTO 1000
       ENDIF
C
...
C
      COMMON /NYTURB/ LNY,KLNY(NNLEV),LDIST,KLDIST(NNLEV),
     *                LWAND,KLWAND(NNLEV)
c
...
c
      ENDIF
c
       CALL MATML1(DAX(1),DAX(I2),DAX(IP),DX(1),DX(I2),DX(IP),A1,A2,
     *            VWORK(KA1),KWORK(KCOLA),KWORK(KLDA),
     *            VWORK(KB1+NB),VWORK(KB2+NB),KWORK(KCOLB),KWORK(KLDB),
     *            NU,NP,KWORK(KMBD),NMBD,INEUM)
C
       CALL MATML2(DAX(1),DAX(I2),DAX(IP),DX(1),DX(I2),DX(IP),A1,A2,
     *            VWORK(KA1),KWORK(KCOLA),KWORK(KLDA),
...
c      PARAMETER (NNARR=299, NNLEV=9, NNWORK=1)
...
C
      IF (ISL.EQ.1) THEN
C
c      IF ((ISL.EQ.1).or.(isl.eq.10)) THEN
...
c
      IF (ISL.EQ.2) THEN
c      IF ((ISL.EQ.2).or.(isl.eq.10)) THEN
...
C
       ...
       RHO=(RES/RESINI)**(1D0/DBLE(max(1,ITE-1)))
ccc       write(6,1001) ite,res,DMPSLC*RESINI,RHO
c$$$       IF ((RES.LT.DEFSLC).AND.(RES.LT.DMPSLC*RESINI)) GOTO 311
        IF ((RES.LT.DMPSL*RESINI)) GOTO 311
C
       CALL LCP1(DX,DD,NUP)
       CALL LCL1(DX,NUP)
       CALL VANCR2 (DX(1),DX(I2),DX(IP),DB(1),DB(I2),DB(IP),
     *            VWORK(KA1),KWORK(KCOLA),KWORK(KLDA),
     *            VWORK(KB1),VWORK(KB2),
     *            VWORK(KB1+NB),VWORK(KB2+NB),KWORK(KCOLB),KWORK(KLDB),
     *            KWORK(KMBD),KWORK(KVERT),KWORK(KMID),DWORK(L(LCORVG)),
     *            KWORK(KNPR),NMBD,1,1,ite)
C
       IF (INEUM.EQ.0) CALL TOL20A(DX(IP),VWORK(KAREA),NP,INEUM)
C
       CALL  LLC1 (DD,DX,NUP,1D0,RLXSL)
C
310    CONTINUE
C
311    CALL LCP1(DWORK(L(LRHS1)),DB,NUP)
C
cc       if (MT.GE.2) write(6,1001) ite,res,DMPSLC*RESINI,RHO
C
...
C
c--------------------------------------------------
c   BEI ISL=3 wird auf dem feinsten Level direkt geloest!
c--------------------------------------------------
      IF (ISL.EQ.3) THEN
c
...
c      IF ((ISL.EQ.11).or.(isl.eq.101)) THEN
      IF ((ISL.EQ.10).or.(ISL.EQ.3)) THEN
       CALL ZNEW(NUP,-1,LRHS1,'DRHS1 ')
       IF (IER.NE.0) GOTO 99999
C
       CALL LCP1 (DB,DWORK(L(LRHS1)),NUP)
...
C
       ...
       RHO=(RES/RESINI)**(1D0/DBLE(max(1,ITE-1)))
ccc       write(6,1001) ite,res,DMPSLC*RESINI,RHO
       IF ((RES.LT.DMPSL*RESINI)) GOTO 411
C
       CALL LCP1(DX,DD,NUP)
       CALL LCL1(DX,NUP)
c 
...
C
c$$$       if (MT.GE.2) write(6,1001) ite,res,DMPSLC*RESINI,RHO
C
       CALL ZDISP(0,LRHS1,'DRHS1 ')
       IF (IER.NE.0) GOTO 99999
C
...
C
C************************************************************************
      SUBROUTINE   YEXVK (DX,DB,DD,NEQ,RHO)  
************************************************************************
*
*   Purpose: - computes on level ILEV on DX the solution of
*              
...
C
       CALL  VANCAE (DX(1),DX(I2),DX(IP),DD(1),DD(I2),DD(IP),
     *               DB(1),DB(I2),DB(IP),
     *               VWORK(KA1+4*NA),KWORK(KCOLA),KWORK(KLDA),
     *               VWORK(KB1),VWORK(KB2),KWORK(KCOLB),KWORK(KLDB),
     *               NU,NP,KWORK(KMBD),KWORK(KVERT),
     *               KWORK(KMID),KWORK(KNPR),NMBD,RLXSL,DMAXU,DMAXP)
C
...
C
     ...
     *            KWORK(KMBD),KWORK(KVERT),KWORK(KMID),DWORK(L(LCORVG)),
     *            KWORK(KNPR),NMBD,1,1,ite)
c     *              KWORK(KMBD),KWORK(KVERT),KWORK(KMID),KWORK(KNPR),
c     *              NMBD,1,1)
C
       IF (INEUM.EQ.0) CALL TOL20A(DX(IP),VWORK(KAREA),NP,INEUM)
C
...
c$$$C=======================================================================
c$$$          CALL GARLI3 (ILEV,Npc,NVTc,NMTc,
c$$$     *             DWORK(KCORC), KWORK(KVERTc),KWORK(KMIDc),
c$$$     *             DUc(1),DUc(I2C),89)
c$$$C=======================================================================
C
...
c$$$     *             DWORK(L(LCORVG)), KWORK(KVERTF),KWORK(KMIDF),
c$$$     *             DUF(1),DUF(I2F),89)
c$$$C=======================================================================
C
      END
...
C
c$$$C=======================================================================
c$$$          CALL GARLI3 (ILEV,NPF,NVTF,NMTF,
c$$$     *             DWORK(KCORF), KWORK(KVERTf),KWORK(KMIDf),
c$$$     *             DDf(1),DDf(I2f),79)
c$$$C=======================================================================
C
...
c$$$C=======================================================================
c$$$          CALL GARLI3 (ILEV,NEL,NVT,NMT,
c$$$     *             DWORK(L(LCORVG)), KWORK(KVERTC),KWORK(KMIDC),
c$$$     *             DDC(1),DDC(I2C),79)
c$$$C=======================================================================
      END
C
...
C=======================================================================
C
      IF (ISM.EQ.1) THEN! vanca mit Diag, aber auf Defektbasis
C
       CALL ZNEW(NUP,-1,LRHS1,'DRHS1 ')
       IF (IER.NE.0) GOTO 99999
C
...
C
      ENDIF
c
      IF (ISM.EQ.2) THEN !Vanca, mit VOLLER Elementmatrix, 
C
       CALL ZNEW(NUP,-1,LRHS1,'DRHS1 ')
       IF (IER.NE.0) GOTO 99999
C
...
C
       ENDIF
c      DIMENSION A1(NVELO,NVELO)

      IF (ISM.EQ.3) THEN !Vanca, mit VOLLEN Diagonalmatizen
C                        A1 (fuer u) und A2 (fuer v)
C                        Mittelding zwischen Ism=1 und ISM=2
       CALL ZNEW(NUP,-1,LRHS1,'DRHS1 ')
       IF (IER.NE.0) GOTO 99999
C
       CALL LCP1 (DB,DWORK(L(LRHS1)),NUP)
...
C
************************************************************************
      SUBROUTINE  YSM1 (DX,DB,DD,NEQ,NSMO)  
************************************************************************
*
*   Purpose: - performs NSMO smoothing steps applied to the system
*                          A*DX = DB
...
c$$$     *             Dwork(l(lrhs)),Dwork(l(lrhs)+NU),30+ite)
c$$$          ENDIF
c$$$C=======================================================================
C
       DO 310 ITE=1,NSMO
...
C
c$$$       if (dby.eq.0d0) then
c$$$          write (*,*) 'DD und LD'
c$$$          write (*,*) (i,dd(i), dwork(l(ld1)-1+i),'\n',i=1,nup)
c$$$          endif
C
...
c       ALPHA=DBY/DBX
       ...
       WRITE(*,*)'ALPHA=====',ALPHA,ilev,dby,dbx!DBY,DBX,ILEV
