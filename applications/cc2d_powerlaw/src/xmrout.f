...
c
************************************************************************
C    Hier wird die anisotrope Verfeinerung gesteuert
************************************************************************
c
c$$$      IF ((BANISO)) THEN
      IF ((BANISO).AND.(ILEV.GE.IREFLV)) THEN
c
          CALL ZNEW(NVBD,3,KLMBD(Ilev),'KMBD  ')
          CALL ZNEW(NVBD,1,KLDBD(Ilev),'DDBD  ')
c
          CALL NEUTST( KWORK(L(KLMBD(ilev))),KWORK(L(LVBD)),
     *            KWORK(L(LEBD)),KWORK(L(LVERT)),
     *            KWORK(L(LMID)),KWORK(L(LNPR)),
     *            DWORK(L(KLDBD(ilev))),DWORK(L(LMBDP)),NMBD)

          KNMBD(Ilev)=NvbD      
C     Distance to the wall and number of next wall element
C     are calculated and stored in LDIST and LWAND respectively
c
          CALL ZNEW (NEL,1,LDIST,'LDIST ')
          CALL ZNEW (NEL,3,LWAND,'LWAND ')
c

          CALL RATEST (DWORK(L(LCORVG)),KWORK(L(LVERT)),KWORK(L(LMID)),
     *            KWORK(L(LVBD)),KWORK(L(LEBD)),KWORK(L(LWAND)))
c

          CALL REFANI (DWORK(L(LCORVG)),KWORK(L(LVERT)),KWORK(L(LWAND)),
     *             NEL,i1,i2,i3,i4)
...
c   
      ENDIF !(BANISO)
************************************************************************
c      IF (ILEV.GE.NLMIN) THEN
      IF (ILEV.GE.1) THEN
C
C *** Save dimensions for all levels between NLMIN and NLMAX
C
C=======================================================================
C *** calcul. of a vector with the Aspect Ratios of all finite elements
C=======================================================================
c
      KLDIST(ILEV)=LDIST
      KLWAND(ILEV)=LWAND
...
C
C
c     *** Ausgabe des Aspectratios und Ereugung des Vektors
C      LASPR auf Level ILEV
c
      IF (ILEV.EQ.NLMIN) WRITE (*,1000) 
      IF (ILEV.EQ.NLMIN) WRITE (*,1002) 

C 
      laspr=0
      CALL ZNEW(NEL,1,LASPR,'VASPR ')
      IF (IER.NE.0) GOTO 99999
c
      KLASPR(ILEV)=LASPR
c 
         CALL ARCLC2 (ARMAX,HMIN,HMAX,NEL,
     *                KWORK(L(LVERT)),DWORK(L(LCORVG)),
     *                DWORK(l(KLASPR(ILEV))))
c
       WRITE (*,1001) ILEV, HMIN, HMAX , ARMAX
      IF (ILEV.EQ.NLMAX) WRITE (*,1002) 
C
10    CONTINUE
C
 1000 FORMAT (2x,'ilev',4x,'hmin',8x,'hmax',8x,'armax')
 1001 FORMAT (1I5,3(D12.4))!,D)
 1002 FORMAT (40('-'))
C
C
99999 END
...
C
      SUBROUTINE MKKMEL(KMEL,KMID,NEL,NMT,NVT)
C
C	The purpose of this routine is
C	to determine on all levels the numbers 
C	of the elements adjacent to each midpoint.
C	These numbers are stored on KMEL(2,NMT)
