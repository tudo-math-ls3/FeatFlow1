...
/*-----------------------------------------------------*
 *                                                     *
 *   ****  ucd_graph Module Compute Func  ****        *
 *                                                     *
 *-----------------------------------------------------*/

ucd_graph (ucd_input, ucd_output, scale, average ) 
     ...
     int average;
{
  ...
  min_extent[3], max_extent[3], vcolors[12][3], iso_level,
  hi_val, lo_val, val, *min_node_data, *max_node_data, *node_data, *cell_data, 
  *new_cell_data,*xc, *yc, *zc,*zc_copy,sum;
  ...
  *cell_list, cell_tsize, node_csize, ucd_flags, num_vcomp, *nd_comp_list, *cl_comp_list,
  ...
  if (!UCDstructure_get_header (ucd_input, model_name, &data_len, &ucd_flags,
    ...
    AVSerror ("Error in ucd_graphold: can't get header.\n");
    return (0);
  }
	
  if (node_len) { 
    if (!UCDstructure_get_node_data (ucd_input, &node_data)) {
      AVSerror ("Error in ucd_graph: can't get node data.\n"); 
      return (0);
    }
    UCDstructure_get_node_components (ucd_input, &nd_comp_list);
    if(nd_comp_list[0]!=1) {
      AVSerror ("Error in ucd_graph: no scalar component\n"); 
      return (0);
    }
  }
  else {
    AVSerror (" Error in ucd_graph:  no nodal data.\n");
    return (0);
  }
  ...
  *ucd_output = (UCD_structure *)UCDstructure_alloc (model_name, data_len, 
  ...
  if (num_new_cells) {
    ...
    zc_copy = (float *)malloc(sizeof(float) * num_nodes);
    ...
    zmin=zmax=node_data[0];
    for(i=0;i<num_nodes;i++)
      {
	zc_copy[i]=*scale*(node_data[i]-sum);
	if(zmin>node_data[i]-sum)
	  zmin=node_data[i]-sum;
	if(zmax<node_data[i]-sum)
	  zmax=node_data[i]-sum;
      }
    UCDstructure_set_node_positions (*ucd_output, xc, yc, zc_copy);

    UCDstructure_get_extent (ucd_input, min_extent, max_extent);
    min_extent[2]=zmin;max_extent[2]=zmax;
...
ucd_graph_desc()
{
  ...
  AVSset_module_name ("ucd graph", MODULE_FILTER);
	
  AVScreate_input_port ("Input", "ucd", REQUIRED);
	
  AVScreate_output_port ("Output", "ucd"); 
	
  param=AVSadd_float_parameter ("Sacle", 1., FLOAT_UNBOUND, FLOAT_UNBOUND);

  param=AVSadd_parameter ("Average", "boolean", 0, 0, 1 );
	
  AVSset_init_proc (ucd_graph_init);
	
  AVSset_destroy_proc (ucd_graph_finis);
	
  AVSset_compute_proc (ucd_graph);
...
AVSinit_modules()
{
  ...
  AVSmodule_from_desc(ucd_graph_desc);
}
#endif
