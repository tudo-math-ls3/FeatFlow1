...
finish:
	...
	@echo ""
	@echo "To apply the example calculations from Chapter 3 of the"
	@echo "Featflow manual"
	@echo " => Follow the instructions in the file "
	@echo "       'applications/examples/readme.txt'"
	@echo ""
...
dist: fixperms purge_all
	...
	(cd .. && tar --exclude CVS --exclude .rem --exclude .\#\* -cvzf FeatflowCD_$(FFVER).tar.gz FeatflowCD)
...
DONELIBS= \
  $(if $(DONEIDS), \
    $(foreach i, $(DONEIDS), \
      echo ' ' $(i:lib-%=%): \
      $(patsubst lib%.a,%,$(shell (cd object/libraries/$i; ls -C)));),\
    echo 'none')
