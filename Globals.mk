...
########################################################################
# The variable FORCE can be used to enforce a special architecture
# string. If you know your PC while the script is not guessing it 
# correctly, you can either modify this file or use this, e.g.
#
#       make build FORCE=pc-pentium4-linux
#
...
APPS= trigen2d trigen3d tr2to3 cc2d pp2d cc3d pp3d \
      cc2d_movbc cc2d_powerlaw cc3d_movbc bouss bouss_powerlaw

# list of all library modules available at the top level
LIBS= feat3d feat2d sysutils lapack blas umfpack2
...
FCFLAGS   = -mcpu=v9 -m64 -pipe
CCFLAGS   = -mcpu=v9 -m64 -pipe
BLASLIB   = 
LAPACKLIB =
endif
...
FCFLAGS   = -march=pentium-m -msse3 -pipe
CCFLAGS   = -march=pentium-m -msse3 -pipe
BLASLIB   = 
LAPACKLIB = 
endif
...
# Apple mac/osx with gcc/g77 3.x
...
ARC=xiar
OPTFLAGS  = -O3 -xN -ipo
FCFLAGS   = -f77rtl -cm -fpe0 -vec_report0
...
CC=gcc
FC=g77
OPTFLAGS  = -O3 -m64 -mmmx -msse -msse2 -m3dnow -mfpmath=sse -ffast-math -fexpensive-optimizations -ffinite-math-only -fgcse -floop-optimize -foptimize-register-move -foptimize-sibling-calls -frename-registers -freorder-blocks -fomit-frame-pointer -funroll-loops -fprefetch-loop-arrays -fsched-interblock -frerun-loop-opt -frerun-cse-after-loop -freorder-functions
...
FC=/usr/local/ifce/bin/ifort
OPTFLAGS  = 
FCFLAGS   = -mcmodel=large -integer_size 64 -double_size 128 -real_size 64 
...
# -fcheck=all
ifeq ($(ID),pc64-x86-linux)
CC=gcc
FC=g77
OPTFLAGS  = -O0
FCFLAGS   = -fallow-argument-mismatch -ffpe-trap=invalid,zero,overflow  
CCFLAGS   = 
BLASLIB   = 
LAPACKLIB = 
endif
...
########################################################################
# g77 -> gfortran -> g95 fallforward strategy
#
# In new GCC distributions, g77 is replaced by gfortran.
# Check if the fortran compiler exists; if not, try gfortran.
########################################################################
ALTFC:=$(shell (which $(FC) 2>/dev/null || echo ""))
ifeq "$(ALTFC)" ""
ALTFC:="$(FC) not found; trying gfortran... "
FC:=$(shell (which $(FC) 2>/dev/null || echo "gfortran"))
else
ALTFC:=""
endif

ALTFC2:=$(shell (which $(FC) 2>/dev/null || echo ""))
ifeq "$(ALTFC2)" ""
ALTFC:=$(ALTFC)$(FC) "not found; trying g95... "
FC:=$(shell (which $(FC) 2>/dev/null || echo "g95"))
endif
...
.id:
	...
	@echo '  C compiler:        ' $(FCC)
	@echo '  Fortran compiler:  ' $(ALTFC)$(FFC)
